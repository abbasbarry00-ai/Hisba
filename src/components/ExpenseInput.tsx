    ...
    <>
      <div className="fixed bottom-8 left-0 right-0 flex justify-center z-40 pointer-events-none">
        <button
          ...
          className="gumroad-button-green pointer-events-auto rounded-full px-10 py-5 text-2xl flex items-center gap-3 shadow-2xl text-foreground"
        >
          <Plus size={32} />
          سجل التكلفة
        </button>
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed bottom-0 left-0 right-0 bg-background border-t-8 border-foreground z-50 rounded-t-[3rem] p-8 md:p-12 max-h-[90vh] overflow-y-auto shadow-[0_-20px_50px_rgba(0,0,0,0.15)]"
            >
              <div className="max-w-md mx-auto relative mt-4">
                <button 
                  onClick={() => setIsOpen(false)}
                  className="absolute -top-12 left-0 p-3 opacity-80 hover:opacity-100 transition-opacity bg-foreground text-background rounded-full border-4 border-foreground"
                >
                  <X size={28} />
                </button>
                
                <h2 className="text-5xl font-black mb-10 text-accent">تكلفة جديدة</h2>
                ...
                {isRecording ? (
                  <div className="flex flex-col items-center justify-center space-y-8 py-10">
                    ...
                    <p className="font-bold animate-pulse text-2xl text-accent">جاري الاستماع...</p>
                    <button 
                  ...
                  <form onSubmit={handleSubmit} className="flex flex-col space-y-10">
                    <div>
                      <label className="block text-2xl font-bold mb-4 opacity-70 text-accent">المبلغ (دينار عراقي)</label>
                      <input 
                        type="text" 
                        inputMode="numeric"
                        pattern="[0-9]*"
                        ...
                        placeholder="0"
                        className="w-full text-8xl font-black outline-none bg-transparent placeholder-gray-300 border-b-8 border-foreground pb-4 focus:border-accent transition-colors text-accent"
                        autoFocus
                      />
                    </div>
                    ...
                    <div className="flex gap-4 pt-4">
                      ...
                      >
                        <span className="ml-3">سجل التكلفة</span>
